<template>
    <div class="free-trial-area">
        <div class="container">
            <div 
                class="free-trial-content bg-color"
                v-if="free !== null"
            >
                <span class="sub-title">{{free.subtitle}}</span>
                <h2>{{free.heading}}</h2>
...
export default {
    ...
    created: async function (){
        const response = await axios.get('https://cms.pbwebvision.in/api/freetrial?populate=*')
        const { data: {attributes} } = response.data
        this.free = attributes
    },
}
</script>